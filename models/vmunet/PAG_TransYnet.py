...
class Attention_gate(nn.Module):
    def __init__(self,F_p, F_m, F_v, F_int):
        ...
        self.W_g1 = nn.Sequential(
            DepthWiseSeparateConvBlock(F_p, F_int, kernel_size=1,stride=1),#nn.Conv2d(F_p, F_int, kernel_size=1,stride=1,padding=0,bias=True),
            nn.BatchNorm2d(F_int)
            )
        #self.W_g1 = DepthWiseSeparateConvBlock(F_p, F_int, kernel_size=1,stride=1)
        
        self.W_x1 = nn.Sequential(
            nn.Conv2d(F_m, F_int, kernel_size=1,stride=1,padding=0,bias=True),#DepthWiseSeparateConvBlock(F_m, F_int, kernel_size=1,stride=1),#
            nn.BatchNorm2d(F_int)
        )
        #self.W_x1 = DepthWiseSeparateConvBlock(F_m, F_int, kernel_size=1,stride=1)
        self.W_g2 = nn.Sequential(
            nn.Conv2d(F_m, F_int, kernel_size=1,stride=1,padding=0,bias=True),#DepthWiseSeparateConvBlock(F_m, F_int, kernel_size=1,stride=1),#
            nn.BatchNorm2d(F_int)
        )  
        #self.W_g2 = DepthWiseSeparateConvBlock(F_m, F_int, kernel_size=1,stride=1)
        self.W_x2 = nn.Sequential(
            nn.Conv2d(F_v, F_int, kernel_size=1,stride=1,padding=0,bias=True),#DepthWiseSeparateConvBlock(F_v, F_int, kernel_size=1,stride=1),#
            nn.BatchNorm2d(F_int)
        )
        #self.W_x2 = DepthWiseSeparateConvBlock(F_v, F_int, kernel_size=1,stride=1)
        self.psi1 = nn.Sequential(
            nn.Conv2d(F_int, 1, kernel_size=1,stride=1,padding=0,bias=True),#DepthWiseSeparateConvBlock(F_int, 1, kernel_size=1,stride=1),#
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        ...
        self.relu = nn.ReLU(inplace=True)
        self.po = nn.Conv2d(in_channels=F_m+F_v,out_channels=F_v,kernel_size=1)
    def forward(self, p, m, v):
        ...
        psi2 = self.psi2(psi2)        
        out = self.po(torch.cat((m*psi1, v*psi2), dim=1))
